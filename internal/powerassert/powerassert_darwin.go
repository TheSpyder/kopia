...
// Both assertion types are needed: BackgroundTask keeps a lid-closed dark wake alive
// and, unlike PreventSystemSleep, is honored on battery; PreventUserIdleSystemSleep
// stops the idle timer on a fully awake machine. Neither can defeat an explicit sleep
// (lid close or the Sleep menu item) - nothing in user space can.
const (
...
// initFrameworks resolves the CoreFoundation and IOKit entry points. purego is used
...
func hold(ctx context.Context, reason string) Release {
	...
	// IOPMAssertionCreateWithName retains the name, so it can be released here.
