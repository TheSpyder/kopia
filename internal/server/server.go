...
// endUpload records the outcome of a finished snapshot. When report is false the snapshot is
// left out of the notification that goes to the user, which is used for snapshots that were
...
func (s *Server) runSnapshotTask(ctx context.Context, src snapshot.SourceInfo, inner func(ctx context.Context, ctrl uitask.Controller, result *notifydata.ManifestWithError) error) error {
	...
	// snapshots that were cut short by the machine going to sleep are excluded from the
	// notification sent to the user - they are retried, and reporting them as errors trains
	// the user to ignore backup notifications.
	reportResult := true
